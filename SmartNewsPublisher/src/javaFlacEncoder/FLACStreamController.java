...
public class FLACStreamController {
  ...
  volatile int minFrameSize = 0;//zero means unknown

  /* maximum frame size seen so far. Used in stream header */
  volatile int maxFrameSize = 0; //zero means unknown
...
/**
  ...
  private void reset() {
    minFrameSize = 0;//zero means unknown
    maxFrameSize = 0;//zero means unknown
    minBlockSize = 0x7FFFFFFF;
    maxBlockSize = 0;
    samplesInStream = 0;
    streamHeaderPos = 0;
  ...
  public void writeBlock(BlockEncodeRequest ber) throws IOException {
    ...
    if(frameSize < minFrameSize || minFrameSize == 0) minFrameSize = frameSize;
  }
