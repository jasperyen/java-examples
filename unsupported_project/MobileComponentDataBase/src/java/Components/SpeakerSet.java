 ...
 * @author Jasper-Yen
 */
public class SpeakerSet extends ComponentSet {

    public SpeakerSet() {
        setComponentName(Speaker.ComponentName);
        setTableName(Speaker.TableName);
        setAllKeyList(Speaker.AllKeyList);
    ...
    public static class Speaker extends Component{
        ...
        static {
            ...
            MainKey = db.speaker_Table.ModelName;
            TableName = db.speaker_Table.TABLE_NAME;
            ProjectTableName = db2.project_speaker_Table.TABLE_NAME;

            List<String> list = new ArrayList();
            list.add(db.speaker_Table.Vendor);
            list.add(db.speaker_Table.ModelName);
            list.add(db.speaker_Table.Description);
            AllKeyList = list;
            ...
            for (Iterator iter = checkMap.entrySet().iterator(); iter.hasNext(); ) {
                ...
                switch (entry.getKey()) {
                    case db.vendor_Table.Vendor : 
                        ...
                        if (!boo)
                            wrongList.add(db.speaker_Table.Vendor);
                        break;
                }
            }
            return wrongList;
