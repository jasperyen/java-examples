 ...
 * @author Jasper-Yen
 */
public class ClickPadSet extends ComponentSet {
    
    public ClickPadSet() {
        setComponentName(ClickPad.ComponentName);
        setTableName(ClickPad.TableName);
        setAllKeyList(ClickPad.AllKeyList);
    ...
    public static class ClickPad extends Component {
        ...
        static {
            ComponentName = "Click Pad";
            MainKey = db.click_pad_Table.ModelName;
            TableName = db.click_pad_Table.TABLE_NAME;
            ProjectTableName = db2.project_click_pad_Table.TABLE_NAME;

            List<String> list = new ArrayList();
            list.add(db.click_pad_Table.Vendor);
            list.add(db.click_pad_Table.ModelName);
            list.add(db.click_pad_Table.Description);
            AllKeyList = list;
            ...
            for (Iterator iter = checkMap.entrySet().iterator(); iter.hasNext(); ) {
                ...
                switch (entry.getKey()) {
                    case db.vendor_Table.Vendor : 
                        ...
                        if (!boo)
                            wrongList.add(db.click_pad_Table.Vendor);
                        break;
                }
            }
            return wrongList;
        ...
        public String getComponentTableRow(List<String> wrongList){
            ...
            for (Object obj : wrongList) {
                switch ((String)obj) {
                    case db.click_pad_Table.Vendor : {
