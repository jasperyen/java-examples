 ...
 * @author Jasper-Yen
 */
public class SensorSet extends ComponentSet {
    
    public SensorSet() {
        setComponentName(Sensor.ComponentName);
        setTableName(Sensor.TableName);
        setAllKeyList(Sensor.AllKeyList);
    ...
    protected Map<String, String> getCheckTableName() {
        ...
        tableMap.put(db.sensor_type_Table.TABLE_NAME, db.sensor_type_Table.Type);
        return tableMap;
    }
    ...
    public static class Sensor extends Component {
        ...
        static {
            ...
            MainKey = db.sensor_Table.ModelName;
            TableName = db.sensor_Table.TABLE_NAME;
            ProjectTableName = db2.project_sensor_Table.TABLE_NAME;

            List<String> list = new ArrayList();
            list.add(db.sensor_Table.Vendor);
            list.add(db.sensor_Table.Type);
            list.add(db.sensor_Table.ModelName);
            list.add(db.sensor_Table.Description);
            AllKeyList = list;
            ...
            for (Iterator iter = checkMap.entrySet().iterator(); iter.hasNext(); ) {
                ...
                switch (entry.getKey()) {
                    case db.vendor_Table.Vendor : 
                        ...
                        if (!boo)
                            wrongList.add(db.sensor_Table.Vendor);
                        break;
                    case db.sensor_type_Table.Type : 
                        for (Object obj : entry.getValue()) {
                            if (Type.equals((String)obj))
                                boo = true;
                        }
                        if (!boo)
                            wrongList.add(db.sensor_Table.Type);
                        break;
                }
            }
            return wrongList;
        ...
        public String getComponentTableRow(List<String> wrongList){
            ...
            for (Object obj : wrongList) {
                switch ((String)obj) {
                    case db.sensor_Table.Vendor : {
                       vtd = "<td style=\"background-color:#FFD78C;\">";
                    }
                    break;
                    case db.sensor_Table.Type : {
